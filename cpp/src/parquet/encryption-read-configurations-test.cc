...
/*
 * This file contains unit-test for reading encrypted Parquet files with
 ...
 * encryption-write-configurations-test.cc test and will be deleted at the end of the
 * unit-test after reading it, while the second resides in
 * parquet-testing/data repository. Those two encrypted files were encrypted using the
 * same encryption configuration.
 * The name of the encrypted file generated by encryption-write-configurations-test.cc
 * test indicates the encryption configuration used to encrypt it as described in the
 * encryption configuration list below. For example,
 * tmp_encryption_config1.parquet.encrypted was encrypted using encryption
...
// configuration:
// one was been generated in encryption-write-configurations-test.cc tests and is deleted
// once the file is read and the second exists
// in parquet-testing/data folder.
TEST_P(TestDecryptionConfiguration, TestDecryption) {
  int encryption_config_num = std::get<0>(GetParam());
  std::string res = "tmp_encryption_config" + std::to_string(encryption_config_num) +
                    ".parquet.encrypted";
  std::string file_name = data_file(res.c_str());
  ...
  file_name = data_file(std::get<1>(GetParam()));
  // Iterate over the decryption configurations and use each one to read the encrypted
  // parqeut file.
  for (unsigned index = 0; index < vector_of_decryption_configurations_.size(); ++index) {
    unsigned decryption_config_num = index + 1;
...
INSTANTIATE_TEST_CASE_P(
    ...
    ::testing::Values(
        std::make_tuple(1, "uniformEncryption.parquet.encrypted"),
        std::make_tuple(2, "encryptTwoColumnsAndTheFooter.parquet.encrypted"),
        std::make_tuple(3, "encryptTwoColumnsWithPlaintextFooter.parquet.encrypted"),
        std::make_tuple(4, "encryptTwoColumnsAndFooterWithAadPrefix.parquet.encrypted"),
        std::make_tuple(
            5, "encryptTwoColumnsAndFooterDisableAadPrefixStorage.parquet.encrypted"),
        std::make_tuple(6, "encryptTwoColumnsAndFooterUsingCTR.parquet.encrypted")));
